...
const QQDemo: React.FC = () => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const { signInWithQQ, loading, error } = useAuthStore();
  ...
  const handleLogin = async () => {
    ...
    const result = await signInWithQQ('demo_code', state);
    if (result.success) {
      navigate('/');
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md mx-auto bg-white rounded-lg shadow-xl">
        <div className="text-center p-6 border-b">
          <h2 className="text-2xl font-bold text-blue-600">QQ登录演示</h2>
          <p className="text-gray-600 mt-2">
            这是QQ登录的演示页面，将在 {countdown} 秒后自动登录
          </p>
        </div>
